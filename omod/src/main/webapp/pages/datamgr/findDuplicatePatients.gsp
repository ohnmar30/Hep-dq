<%
	...
	ui.includeJavascript("chaidq", "controllers/merge.js")

	def menuItems = [
			[ label: "Select patients manually", iconProvider: "chaidq", icon: "buttons/patient_merge.png", href: ui.pageLink("chaidq", "datamgr/mergePatients", [ returnUrl: ui.thisUrl() ]) ],
	...
	jQuery(function() {
		jQuery('#merge_selected').click(function() {
			...
			if (patientIds.length == 2) {
				ui.navigate('chaidq', 'datamgr/mergePatients', { patient1: patientIds[0], patient2: patientIds[1], returnUrl: location.href });
			}
			else {
				chaiui.notifyError('Please select exactly 2 patients to merge');
			}
		});
	});
</script>
			...
			<table style="width: 100%">
				<tr>
					...
					<td style="text-align: right">
						<button type="button" id="merge_selected"><img src="${ ui.resourceLink("chaiui", "images/glyphs/ok.png") }" /> Merge selected</button>
					</td>
				</tr>
			</table>
		</div>
