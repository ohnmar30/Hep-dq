 ...
 */
chaiemrApp.controller('DuplicatePatients', ['$scope', '$http', function($scope, $http) {
	...
	$scope.refresh = function() {
		...
		$http.get(ui.fragmentActionLink('chaidq', 'patient/mergeUtils', 'getDuplicatePatients', params))
			.success(function(data) {
				$scope.results = data;
				$scope.loading = false;
			})
			.error(function(data) {
				chaiui.notifyError(data.globalErrors.join('<br/>'));
				$scope.loading = false;
			});
	};
}]);