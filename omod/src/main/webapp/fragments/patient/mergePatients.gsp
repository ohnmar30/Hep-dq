<%
	ui.decorateWith("chaiui", "panel", [ heading: "Merge Patients", frameOnly: true ])
%>

<form id="merge-patients-form" method="post" action="${ ui.actionLink("chaidq", "patient/mergePatients", "merge") }">

	<div class="ke-panel-controls">
		<button type="button" class="ke-compact" onclick="switchPatients()"><img src="${ ui.resourceLink("chaiui", "images/glyphs/switch.png") }" /> Switch</button>
	</div>
	...
	<div class="ke-panel-content">
		...
		<table style="width: 100%">
			...
			<tr>
				<td style="text-align: center">${ ui.includeFragment("chaiui", "widget/field", [ id: "patient1-select", object: command, property: "patient1" ]) }</td>
				<td style="text-align: center">${ ui.includeFragment("chaiui", "widget/field", [ id: "patient2-select", object: command, property: "patient2" ]) }</td>
			</tr>
			<tr>
	...
	<div class="ke-panel-controls">
		<button type="button" class="merge-button">
			<img src="${ ui.resourceLink("chaiui", "images/glyphs/ok.png") }" /> Merge
		</button>
...
<script type="text/javascript">
	jq(function() {
		...
		chaiui.setupAjaxPost('merge-patients-form', {
	...
	function switchPatients() {
		...
		chaiui.setSearchField('patient1-select', patient2Id);
		chaiui.setSearchField('patient2-select', patient1Id);
	...
	function updatePatientSummary(patientId, position) {
		...
		ui.getFragmentActionAsJson('chaidq', 'patient/mergePatients', 'patientSummary', { patientId : patientId }, function (patient) {
