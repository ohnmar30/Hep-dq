...
package org.openmrs.module.chaidq.fragment.controller.patient;
...
import org.openmrs.module.chaidq.DqConstants;
import org.openmrs.module.chaidq.api.ChaiDqService;
...
public class MergePatientsFragmentController {
	...
	public Object merge(@MethodParam("newMergePatientsForm") @BindParams MergePatientsForm form,
		...
		try {
			Context.getService(ChaiDqService.class).mergePatients(form.getPatient1(), form.getPatient2());

			chaiUi.notifySuccess(session, "Patients merged successfully");
		}
	...
	public SimpleObject patientSummary(@RequestParam("patientId") Patient patient,
		...
		infopoints.add(dataPoint("Birthdate", chaiUi.formatDate(patient.getBirthdate())));
		infopoints.add(dataPoint("Death date", chaiUi.formatDate(patient.getDeathDate())));
		infopoints.add(dataPoint("Created", chaiUi.formatDate(patient.getDateCreated())));
		infopoints.add(dataPoint("Modified", chaiUi.formatDate(patient.getDateChanged())));
		...
		for (Encounter encounter : Context.getEncounterService().getEncountersByPatient(patient)) {
			...
			encounters.add(dataPoint(chaiUi.formatDate(encounter.getEncounterDatetime()), sb.toString()));
		}
